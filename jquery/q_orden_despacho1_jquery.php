    ...
    switch ($opcion) {
        case 99: //SELECCION DE DATOS
            ...
            SELECT COUNT[[*]] as TOTAL_ROW
             FROM Q_ORDEN
            WHERE 1= 1 AND ESTADO IN ( 'FINALIZADO','DESPACHADO') ";
            ...
            CONCAT('Fecha Despacho Bodega : ',DESPACHO_FECHA,'<br>Usuario Despacho Bodega : ',DESPACHO_ID_USUARIO) AS DESPACHO
...
//                 MODIFICAR ESTE CODIGO PARA EL REPORTE EN ESPAÑOL
        ...
        case 12:
            ...
            $PARAM_ESTADO = vd("FINALIZADO", "text");

            $SQL = "SP_SELECT_Q_ORDEN_FINALIZAR_DESPACHO_ESTADO($PARAM_ID,$user_vd)";
            $cn->execute($SQL, 0);
            if ($cn->filas_afectada() > 0) {
                $cc                      = $cn->datos();
                $matriz["fila_afectada"] = $cc[0]["FILA_AFECTADA"];
        ...
        case 13:
            ...
            $SQL = "SP_SELECT_Q_ORDEN_FINALIZAR_DESPACHO_ESTADO_ANULAR($PARAM_ID,$user_vd)";
            $cn->execute($SQL, 0);
            if ($cn->filas_afectada() > 0) {
                $cc                      = $cn->datos();
                $matriz["fila_afectada"] = $cc[0]["FILA_AFECTADA"];
