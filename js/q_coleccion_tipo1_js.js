 ...
 *        
 *      - URL: $.post("jquery/q_coleccion_tipo1_jquery.php", {opcion: 22,estado: estado}}}
...
$('document').ready(function() {
    ...
    $('#dg_PASO1').datagrid({
        url: 'jquery/q_coleccion_tipo1_jquery.php?opcion=99', // URL para obtener los datos iniciales
    });
...
function autocarga_objetivo_estrategico() {
    ...
    $("#ID_OBJETIVO_ESTRATEGICO").combobox({
        url: "jquery/q_coleccion_tipo1_jquery.php?opcion=11&all_rows=1",
        valueField: "ID",
        textField: "NOMBRE",
...
function open_dialog_PAC_REVISION_new() {
    ...
    url = "jquery/q_coleccion_tipo1_jquery.php?opcion=1&ID_PERIODO=" + $("#ID_PERIODO").val();
}
...
function dg_delete_row_PAC_REVISION() {
    ...
    if (row) {
        $.messager.confirm("Confirmar", "Está seguro de eliminar este registro?", function(r) {
            if (r) {
                $.post("jquery/q_coleccion_tipo1_jquery.php", {
                    ID: row.ID,
                    opcion: 4,
    ...
    if (i > 0) {
        $.messager.confirm("Confirmar", "Está seguro de aprobar los registros seleccionados ?", function(r) {
            if (r) {
                ...
                datos.rows = ids;
                $.post("jquery/q_coleccion_tipo1_jquery.php", datos, function(result) {
                    ms("Info", result.mensaje);
                    if (result.fila_afectada > 0) {
                        $(".dg_PASO1").datagrid("reload");
                    }
...
function noaprobado_individual() {
    ...
    if (i > 0) {
        $.ajax({
            url: 'jquery/q_coleccion_tipo1_jquery.php?opcion=7&rows=' + ids, // Script para generar y enviar el token
            method: 'POST',
            data: {
...
function paso1_grabar_motivo() {
    ...
    if (estado_interno == 1) {
        ...
        if (id_certificacion > 0) {
            $.ajax({
                url: 'jquery/q_coleccion_tipo1_jquery.php?opcion=23&id_certificacion=' + id_certificacion, // Script para generar y enviar el token
                method: 'POST',
                data: {
...
function q_open_nuevo() {
    ...
    url = "jquery/q_coleccion_tipo1_jquery.php?opcion=1";
}

function q_usuario_grabar_nuevo() {
...
function q_edit_estado(estado) {
    ...
    if (row) {
        ...
        url_estado = "jquery/q_coleccion_tipo1_jquery.php?opcion=11&ID=" + row.ID+"&ESTADO="+estado;
...
function q_open_dialog_edit() {
    ...
    if (row) {
        ...
        url = "jquery/q_coleccion_tipo1_jquery.php?opcion=3&id=" + row.ID;
        $(".div_clave").hide();
    } else {
        ms("Info", "DEBE SELECCIONAR UN REGISTRO")
    }
