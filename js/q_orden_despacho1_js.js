 ...
 *        
 *      - URL: $.post("jquery/q_orden_despacho1_jquery.php", {opcion: 22,estado: estado}}}
...
$('document').ready(function() {
    ...
    $('#dg_PASO1').datagrid({
        url: 'jquery/q_orden_despacho1_jquery.php?opcion=99', // URL para obtener los datos iniciales
    });
    $('#dg_PASO2').datagrid({
...
function autocarga_objetivo_estrategico() {
    ...
    $("#ID_OBJETIVO_ESTRATEGICO").combobox({
        url: "jquery/q_orden_despacho1_jquery.php?opcion=11&all_rows=1",
        valueField: "ID",
        textField: "NOMBRE",
...
function q_open_dialog_edit() {
    ...
    if (row) {
        if (row.ESTADO == "BORRADOR") {
            ...
            url = "jquery/q_orden_despacho1_jquery.php?opcion=3&id=" + row.ID;
        } else {
...
function open_dialog_PAC_REVISION_new() {
    ...
    url = "jquery/q_orden_despacho1_jquery.php?opcion=1&ID_PERIODO=" + $("#ID_PERIODO").val();
}
...
function dg_delete_row_PAC_REVISION() {
    ...
    if (row) {
        $.messager.confirm("Confirmar", "Está seguro de eliminar este registro?", function(r) {
            if (r) {
                $.post("jquery/q_orden_despacho1_jquery.php", {
                    ID: row.ID,
                    opcion: 4,
    ...
    if (i > 0) {
        $.messager.confirm("Confirmar", "Está seguro de aprobar los registros seleccionados ?", function(r) {
            if (r) {
                ...
                datos.rows = ids;
                $.post("jquery/q_orden_despacho1_jquery.php", datos, function(result) {
                    ms("Info", result.mensaje);
                    if (result.fila_afectada > 0) {
                        $(".dg_PASO1").datagrid("reload");
                    }
...
function noaprobado_individual() {
    ...
    if (i > 0) {
        $.ajax({
            url: 'jquery/q_orden_despacho1_jquery.php?opcion=7&rows=' + ids, // Script para generar y enviar el token
            method: 'POST',
            data: {
...
function paso1_grabar_motivo() {
    ...
    if (estado_interno == 1) {
        ...
        if (id_certificacion > 0) {
            $.ajax({
                url: 'jquery/q_orden_despacho1_jquery.php?opcion=23&id_certificacion=' + id_certificacion, // Script para generar y enviar el token
                method: 'POST',
                data: {
...
function q_open_nuevo_orden() {
    ...
    url = "jquery/q_orden_despacho1_jquery.php?opcion=1";
...
function q_anular_orden(estado) {
    ...
    if (row) {
        if (row.ESTADO == "BORRADOR") {
            $.messager.confirm("Confirmar", "Está seguro de eliminar este registro?", function(r) {
                if (r) {
                    url_estado = "jquery/q_orden_despacho1_jquery.php?opcion=11&ID=" + row.ID + "&ESTADO=" + estado;
                    $.post(url_estado, {
                        param1: 'value1'
...
function estado_color(value, row) {
    if (!value) return ''; // Si no hay imagen, no mostrar nada
    if (value == "FINALIZADO") {
        return '<div style="background:red">PENDIENTE</div>';
    } else {
        return '<div style="background:green;font-weight:bold">' + value + '</div>';
    }
...
function q_finalizar_despacho_orden(estado) {
    var row = $("#dg_PASO1").datagrid("getSelected");
    if (row) {
        if (row.ESTADO == "FINALIZADO") {
            $.messager.confirm("Confirmar", "Está seguro de realizar el despacho de esta orden ?", function(r) {
                if (r) {
                    url_estado = "jquery/q_orden_despacho1_jquery.php?opcion=12&ID=" + row.ID + "&ESTADO=" + estado;
                    $.post(url_estado, {
                        param1: 'value1'
                    }, function(data, textStatus, xhr) {
                        $("#dg_PASO1").datagrid("reload");
            ...
            ms("Info", "Debe tener una orden FINALIZADO");
        }
    }
}

function q_finalizar_despacho_orden_anular(estado) {
    var row = $("#dg_PASO1").datagrid("getSelected");
    console.log(row);
    if (row) {
        if (row.ESTADO == "FINALIZADO") {
            $.messager.confirm("Confirmar", "Está seguro de realizar la anulación de esta orden ?", function(r) {
                if (r) {
                    url_estado = "jquery/q_orden_despacho1_jquery.php?opcion=13&ID=" + row.ID + "&ESTADO=" + row.ESTADO;
                    $.post(url_estado, {
                        param1: 'value1'
                    }, function(data, textStatus, xhr) {
                        $("#dg_PASO1").datagrid("reload");
